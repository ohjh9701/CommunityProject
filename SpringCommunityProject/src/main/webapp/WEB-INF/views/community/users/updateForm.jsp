...
<head>
    ...
    <title>Arsenal FC - Update Profile</title>
    ...
    <style>
        ...
        .form-label { font-weight: bold; color: #063672; }
        
        /* 아이디는 수정 불가하므로 배경색 처리 */
        .form-control-plaintext { background-color: #f1f1f1; padding: 10px; border-radius: 5px; color: #666; font-weight: bold; }

        .btn-arsenal { background-color: #EF0107; color: white; border: none; padding: 12px; font-weight: bold; transition: 0.3s; }
        .btn-arsenal:hover { background-color: #DB0007; transform: translateY(-2px); }
        .btn-cancel { background-color: #6c757d; color: white; border: none; padding: 12px; font-weight: bold; }
...
<div class="main-content">
    <div class="update-container">
        <div class="brand-logo">UPDATE PROFILE</div>
        <p class="instruction">회원 정보를 수정하실 수 있습니다.</p>

        <form action="/community/updateUser" method="post">
            <input type="hidden" name="no" value="${loginUser.no}">

            <div class="mb-3">
                <label class="form-label">아이디 (변경 불가)</label>
                <input type="text" name="id" class="form-control-plaintext" value="${loginUser.id}" readonly>
            </div>

            <div class="mb-3">
                <label for="password" class="form-label">비밀번호</label>
                <input type="password" id="password" name="password" class="form-control" value="${loginUser.password}" required>
            </div>
            
            <div class="mb-3">
                <label for="nickName" class="form-label">닉네임</label>
                <input type="text" id="nickName" name="nickName" class="form-control" value="${loginUser.nickName}" required>
            </div>

            <div class="mb-4">
                <label for="email" class="form-label">이메일</label>
                <input type="email" id="email" name="email" class="form-control" value="${loginUser.email}" required>
            </div>
            <div class="d-grid gap-2">
                <button type="submit" class="btn btn-arsenal">정보 수정 완료</button>
                <a href="/community/mypage" class="btn btn-cancel">취소</a>
            </div>
        </form>
    </div>
</div>
