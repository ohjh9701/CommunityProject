...
<head>
    ...
    <style>
        ...
        .board-container { background: white; padding: 40px; border-radius: 15px; box-shadow: 0 5px 20px rgba(0,0,0,0.05); }
        ...
        .top-search-area .form-control:focus { box-shadow: none; }

        /* 테이블 스타일 */
        .table thead { background-color: #fcfcfc; border-bottom: 2px solid var(--ars-red); }
        .table th { color: var(--ars-blue); font-weight: 700; }
        .table td { vertical-align: middle; }
        .title-cell { text-align: left !important; }

        /* [추가] 페이징 스타일 */
        .pagination .page-link { color: var(--ars-blue); border: 1px solid #dee2e6; }
        .pagination .page-item.active .page-link { background-color: var(--ars-red); border-color: var(--ars-red); color: white; }
        .pagination .page-link:hover { background-color: #f8f9fa; color: var(--ars-red); }

        .btn-arsenal { background-color: var(--ars-red); color: white; border: none; font-weight: bold; border-radius: 5px; }
            ...
            <nav class="mt-5">
                <ul class="pagination justify-content-center">
                    <li class="page-item ${paging.prev ? '' : 'disabled'}">
                        <a class="page-link" href="?page=${paging.startPage - 1}&searchType=${board.searchType}&keyword=${board.keyword}">&laquo;</a>
                    </li>
                    
                    <c:forEach var="num" begin="${paging.startPage}" end="${paging.endPage}">
                        <li class="page-item ${paging.currentPage == num ? 'active' : ''}">
                            <a class="page-link" href="?page=${num}&searchType=${board.searchType}&keyword=${board.keyword}">${num}</a>
                        </li>
                    </c:forEach>

                    <li class="page-item ${paging.next ? '' : 'disabled'}">
                        <a class="page-link" href="?page=${paging.endPage + 1}&searchType=${board.searchType}&keyword=${board.keyword}">&raquo;</a>
                    </li>
                </ul>
            </nav>
        </div>
