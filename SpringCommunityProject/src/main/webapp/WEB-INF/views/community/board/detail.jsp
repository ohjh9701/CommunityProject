...
.detail-header {
	...
	color: white;
	padding: 40px;
	border-bottom: 5px solid var(--ars-red);
}
		...
		<div class="container">
			<div class="detail-card">
				<div class="detail-header">
					...
					<div class="post-info">
						<span>작성자: <b>${board.user.nickName}</b></span> <span>작성일:
							<b><fmt:formatDate value="${board.regDate}"
									pattern="yyyy.MM.dd HH:mm" /></b>
						</span> <span>NO: <b>${board.no}</b></span>
					</div>
				</div>
				...
				<div class="container mt-5">
					...
					<c:if test="${not empty loginUser}">
						<div class="card mb-4 border-0 shadow-sm">
							<div class="card-body">
								<form action="/community/replies/insert" method="post">
									<input type="hidden" name="boardNo" value="${board.no}">
									<input type="hidden" name="usersId" value="${loginUser.id}">

									<div class="input-group">
										<textarea name="content" class="form-control" rows="2"
											placeholder="구너들의 소중한 댓글을 남겨주세요!" required></textarea>
										<button class="btn btn-danger px-4" type="submit">등록</button>
									</div>
								</form>
							</div>
						</div>
				...
				<div class="detail-footer">
					...
					<div class="d-flex gap-2">
						...
						<c:if test="${loginUser.id eq board.user.id}">
							<a href="/community/board/updateForm?no=${board.no}"
								class="btn btn-danger fw-bold">수정</a>
							<a href="/community/board/delete?no=${board.no}"
